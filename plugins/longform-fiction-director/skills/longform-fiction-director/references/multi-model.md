...
4. 写稿时先询问，作者当次选择使用后才以 authorConfirmed=true 调用 fiction_generate_to_file：完整生成 → 写到 Codex候选/*.txt → 再 fiction_read_artifact 读取。
...
|---|---|---|
| 脑洞/市场 | 探索 | flash / luna |
| 大纲/细纲/控制卡 | Codex 总责编默认本地完成；需要第二意见才调用 | terra / sonnet / kimi / glm |
...
- 作者拒绝时不抢写，先问继续磨控制卡还是由 Codex 写临时候选
