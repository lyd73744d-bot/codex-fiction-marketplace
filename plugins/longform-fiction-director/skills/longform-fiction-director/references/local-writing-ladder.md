...
绑定 → 脑洞 → 可选章节笔记 → 候选正文 → 去 AI 味（可选，先定级）→ 质检（按实际问题）→ 确认入台账
...
只有一整章完成、一轮质检完成，或作者主动问进度时，才按 `delivery-dashboard.md` 输出短摘要。需要章节笔记时参考 `chapter-control-card.md`，但不做表格，也不把笔记当正文施工命令。质检完成时只展示关键结论。
