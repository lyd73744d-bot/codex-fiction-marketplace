...
function createGatewayMcpTools({ gateway, gatewayGuard, openLoginPage, generationJobs } = {}) {
  ...
  const definitions = [
    ...
    { name: "fiction_recommend_models", description: "Lead-editor router: recommend one external model for a writing task and list manual alternatives. Supports mode=quick|deep. Pass returned modelIds to generate_to_file.modelIds; models are never switched automatically.", annotations: safetyAnnotations(true), inputSchema: { type: "object", additionalProperties: false, properties: { task: { type: "string", maxLength: 64 }, mode: { type: "string", maxLength: 16 }, maxPerRole: { type: "number" }, authorPrefer: { type: "array", items: { type: "string" }, maxItems: 8 } } } },
