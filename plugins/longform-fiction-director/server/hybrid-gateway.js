...
const LEGACY_PREFERRED = new Set(["minimax-m3"]);
...
function createHybridGateway({ primary, secondary, label, allowedModels = [], preferredModel = "", creditsPerCall = 10, balance = -1, modelCredits = null } = {}) {
  ...
  const creditMap = modelCredits && typeof modelCredits === "object" && !Array.isArray(modelCredits)
    ? modelCredits
    : (primary && primary.modelCredits) || {"claude-sonnet-5":10,"claude-opus-4-6":20,"gemini-3.1-pro-preview":10,"doubao-seed-2-1-turbo":10,"glm-5.2":10,"minimax-m3":5,"deepseek-v4-flash":5,"deepseek-v4-pro":10,"kimi-k2.6":10};
