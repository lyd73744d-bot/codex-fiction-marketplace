...
function chapterOrdinal(title) {
  const match = String(title || "").match(/^\s*\u7b2c\s*([0-9\u96f6\u3007\u4e00\u4e8c\u4e24\u4e09\u56db\u4e94\u516d\u4e03\u516b\u4e5d\u5341\u767e\u5343\u4e07]+)\s*\u7ae0/u);
  if (!match) return null;
  if (/^[0-9]+$/u.test(match[1])) return Number(match[1]);
  return parseChineseChapterNumber(match[1]);
}

function sortChaptersByTitleOrdinal(chapters) {
  const annotated = chapters.map((chapter, sourceIndex) => ({
    chapter,
    sourceIndex,
    ordinal: chapterOrdinal(chapter.title)
  }));
  if (annotated.some((item) => !Number.isSafeInteger(item.ordinal) || item.ordinal < 0)) return chapters;
  if (new Set(annotated.map((item) => item.ordinal)).size !== annotated.length) return chapters;
  return annotated
    .sort((left, right) => left.ordinal - right.ordinal || left.sourceIndex - right.sourceIndex)
...
async function readDownloadedBook(jsonlPath, title, charset) {
  ...
  const orderedChapters = sortChaptersByTitleOrdinal(chapters);
  const header = title ? `《${title}》\n\n` : "";
  const text = `${header}${orderedChapters.map((chapter) => `${chapter.title}\n\n${chapter.body}`).join("\n\n\n")}\n`;
  return { text, chapters: orderedChapters };
}
...
function createDownloadProvider({
  ...
  async function search(query) {
    const wantedTitle = String(query || "").trim();
    if (!wantedTitle) {
    ...
    return items.slice(0, 20).map((item) => ({
      bookId: normalizeBookId(item.book_id || item.bookId),
  ...
  async function resolveBook({ title, bookId }) {
    ...
    const items = await search(wantedTitle);
    if (!items.length) throw createError("BOOK_NOT_FOUND", `The downloader did not find ${wantedTitle}.`);
    const selected = items.find((item) => String(item.title || "").trim() === wantedTitle);
    if (!selected) {
      const error = createError("BOOK_SELECTION_REQUIRED", "The title search was not an exact match. Use a Fanqie book id to avoid downloading the wrong book.");
      error.candidates = items.slice(0, 5).map((item) => ({
    ...
    return {
      bookId: normalizeBookId(selected.bookId),
